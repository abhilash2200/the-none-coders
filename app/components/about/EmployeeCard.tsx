...
const overlayVariants: Variants = {
  rest: { opacity: 0, filter: "blur(0px)", scale: 1 },
  hover: {
    opacity: 1,
    filter: "blur(20px)",
    scale: 1.05,
...
const lineVariants: Variants = {
  rest: { opacity: 0, y: 20 },
  hover: { opacity: 1, y: 0, transition: { duration: 0.45, ease: "easeOut" } },
};
...
function EmployeeItem({
  ...
  return (
    ...
    >
      ...
      {/* Overlay */}
      <motion.div variants={overlayVariants} className="absolute inset-0 z-20 pointer-events-none">
      ...
      {/* Content */}
      <motion.div variants={contentVariants} className="absolute inset-0 z-30 flex flex-col justify-center items-start p-6 text-white">
        <motion.h4 variants={lineVariants} className="text-2xl font-bold">{emp.name}</motion.h4>
        <motion.p variants={lineVariants} className="text-sm text-neutral-300 mb-2">{emp.designation}</motion.p>
        <motion.p variants={lineVariants} className="text-sm max-w-md leading-relaxed text-neutral-200">{emp.details}</motion.p>

        <motion.div variants={lineVariants} className="flex gap-4 mt-4">
          {[{ icon: Linkedin, label: "LinkedIn" }, { icon: Twitter, label: "Twitter" }, { icon: Github, label: "GitHub" }].map((s, i) => (
            <motion.a
              key={i}
              href="#"
              aria-label={s.label}
